...
public class CompressorMenu extends AbstractContainerMenu {
    ...
    public CompressorMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, (CompressorBE) inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }
    ...
    @Override public boolean stillValid(Player player) { return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.COMPRESSOR.get()); }
