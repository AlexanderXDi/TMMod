...
public record InductionFurnaceRecipe(Ingredient input1, Ingredient input2, ItemStack output1, ItemStack output2, int heatPerSecond, int time, int euPerTick) implements Recipe<RecipeInput> {
    @Override
    public boolean matches(RecipeInput input, Level level) {
        ItemStack s0 = input.getItem(0);
        ItemStack s1 = input.getItem(1);
        
        if (input2 == Ingredient.EMPTY) {
            return input1.test(s0) || input1.test(s1);
        }
        
        boolean direct = input1.test(s0) && input2.test(s1);
        boolean swapped = input1.test(s1) && input2.test(s0);
